...
```sh
kubectl create secret generic carto-basemaps \
  --from-literal=api-key='YOUR_CARTO_KEY'
```

The Helm chart references that Secret through `carto.apiKey.existingSecret` in
`helm-chart/values.yaml`. If the Secret value changes, restart the Deployment so
the container regenerates `/config.js` with the new environment variable.
